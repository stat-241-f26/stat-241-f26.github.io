...
* Problem Set 1 due date moved!
    * New date: **Wednesday 9/23** at 10pm.
    
* Problem 5 supplemental instructions on Slack. 
...
babynames_math241 <- babynames %>%
  filter(name %in% c("Maeve", "Panya",
                     "Ezra", "Ida", "Fuxing", "Fae",
                     "John", "Roseth", "Tianrun", "Vasiliy",
                     "Grayson")) %>%
  group_by(year, name) %>%
  summarize(n = sum(n)) %>%
  ungroup() %>%
...
```{r}
#| eval: false
anim_save("my_plots/stat241_an.gif", animate(stat241_anim))
```
...
* **Question**: My RStudio Project is `stat-241-f26.github.io`. Why does the file path end a folder further there when I run the following?
...
* If you end up with a mess of errors, then don't worry but come see me or the course assistant for help.
...
* Under the **stat-241-f26** GitHub Organization you currently have 1 repo:
    + `pset01-username`: Just you and the Stat 241 teaching team can access
...
-   More data wrangling
-   Learn about different data types in `R` (beyond `data.frames`)
...
-   data joins and reshaping data

-   spatial data

-   Interactive dashboards with `shiny` (the subject of Project 1!)
